# A simple circuit breaker to provide retry logic.
...
```bash
php artisan vendor:publish --tag="circuit-breaker-migrations"
php artisan migrate
```
...
```php
$circuitBreaker = new christopheraseidl\CircuitBreaker();
echo $circuitBreaker->echoPhrase('Hello, christopheraseidl!');
```
